...
function UnavailabilityManagerModal({ holidays, onClose, onRefresh }) {
  ...
  const removeEntry = async (date, session) => {
    try {
      ...
      setList(list.filter(h => !(h.date === date && h.session === session)));
    } catch (err) {
        ...
        <div className="p-6 space-y-5 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide mb-3">
              Blocked Dates ({list.length} {list.length === 1 ? "entry" : "entries"})
            </p>
...
export default function DoctorAppointments() {
  ...
  // Determine which session is currently active based on time of day
  // Morning: 07:00–08:00, Evening: 17:00–20:00, between/after → Evening
  const currentSession = (() => {
    const now = new Date();
    const mins = now.getHours() * 60 + now.getMinutes();
    if (mins < 8 * 60) return "Morning";   // before 08:00 → morning
    ...
    <DoctorLayout activePage="My Schedule">
      ...
      <div className="p-6 space-y-5">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            { label: "Total Today", value: loadingAppts ? "…" : todayAppts.length, color: "#1565C0", bg: "#E3F2FD" },
            { label: "Completed",   value: loadingAppts ? "…" : completed,          color: "#00897B", bg: "#E0F2F1" },
            { label: "In Progress", value: loadingAppts ? "…" : inProgress,         color: "#E65100", bg: "#FFF3E0" },
