...
function Toast({ msg, type, onDone }) {
  ...
  return (
    <div className={`fixed top-5 right-5 z-[200] flex items-center gap-3 px-5 py-3 rounded-2xl
      text-white shadow-2xl text-sm font-medium ${type === "success" ? "bg-emerald-600" : "bg-red-500"}`}
...
// ══════════════════════════════════════════════════════════════
//  DISPENSE MODAL  (3-phase: review → confirm → done)
// ══════════════════════════════════════════════════════════════
function DispenseModal({ rx: rxProp, onClose, onDone }) {
  ...
  const [phase,   setPhase]   = useState("review"); // review | confirm | done
  ...
  const selectDrug = (lineId, drug) => {
    ...
    setLines(p => p.map(l => l._id === lineId
      ? { ...l, drugId: drug._id, _drugLabel: `${drug.name} ${drug.strength}`, _searchText: "", _totalStock: stk }
      : l));
  ...
  const clearDrug   = (lineId) =>
    setLines(p => p.map(l => l._id === lineId
      ? { ...l, drugId: null, _drugLabel: "", _searchText: "", _totalStock: 0 } : l));
  ...
  const allDone    = lines.every(l =>
    l.availability === "available"
      ? (l.drugId && l.qtyToDispense > 0)
      : !!l.pharmacistNote?.trim()
  );
  ...
  const doDispense = async () => {
    ...
    try {
      ...
      // Step 2 — dispense (deducts stock + creates bill)
      const dRes  = await fetch(`${API}/pharmacy/${rxProp._id}/dispense`, {
        method: "POST", headers: jsonH(),
      });
      const dData = await dRes.json();
      if (!dData.success) throw new Error(dData.message);

      setBill(dData.bill || null);
      setPhase("done");
      onDone(dData.pharmacyPrescription);
    } catch (e) {
      setError(e.message);
      setPhase("confirm");
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[93vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-5">
            ...
            <>
              <div className="flex items-center justify-between">
                <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">
                  {lines.length} Medication{lines.length !== 1 ? "s" : ""}
                </p>
                {outLines.length > 0 && (
                  <span className="text-xs text-slate-600 font-semibold bg-slate-50 px-2.5 py-1 rounded-full border border-slate-200">
                    ⚠️ {outLines.length} marked unavailable
                  </span>
                )}
              </div>
              ...
              {lines.map((line, idx) => (
                <div key={line._id} className={`rounded-2xl border-2 p-4 space-y-3 transition ${
                  ...
                  {/* Drug search (available only) */}
                  {line.availability === "available" && (
                    <div className="relative">
                      <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">
                        Inventory Drug <span className="text-red-400">*</span>
                      </label>
                      {line.drugId ? (
                  ...
                  {line.availability === "available" && (
                    <div>
                      ...
                      <input type="number" min="1"
                        ...
                        placeholder="0"
                        className="w-32 px-4 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-slate-400 bg-white" />
                  ...
                  {line.availability !== "available" && (
                    <div>
                      <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">
                        Note for Cashier / Patient <span className="text-red-400">*</span>
                      </label>
                      <input type="text"
                        value={line.pharmacistNote}
                        onChange={e => updateLine(line._id, "pharmacistNote", e.target.value)}
                        placeholder={`e.g. ${line.medicationName} — not available, advise patient to return`}
          ...
          {phase === "confirm" && (
            <>
              {/* Available — will deduct stock */}
              {availLines.length > 0 && (
                <div>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">
                    ✅ Will be Dispensed — Stock Deducted via FEFO
                  </p>
                  <div className="space-y-2">
                    {availLines.map(l => (
                      <div key={l._id}
                        className="flex items-center justify-between bg-slate-50 border border-slate-200 px-4 py-3 rounded-xl text-sm">
                        <div>
                          <span className="font-semibold text-gray-800">{l.medicationName}</span>
                          {l._drugLabel && <span className="text-xs text-gray-400 ml-2">→ {l._drugLabel}</span>}
                        </div>
              ...
              {/* Out of stock — included in bill as unavailable */}
              {outLines.length > 0 && (
                <div>
                  <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">
                    ⚠️ Out of Stock — Listed on Bill (no stock deducted)
                  </p>
                  <div className="space-y-2">
                    {outLines.map(l => (
                      <div key={l._id}
                        className="bg-amber-50 border border-amber-200 px-4 py-3 rounded-xl text-sm">
                  ...
                  <div className="mt-2 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 text-xs text-amber-800">
                    Cashier will see these as unavailable items on the bill and dashboard — patient can be informed.
                  </div>
                </div>
              )}
              ...
              {generalNote && (
                <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-sm">
                  <p className="text-xs font-semibold text-blue-600 uppercase mb-1">Note → Cashier Dashboard</p>
                  <p className="text-gray-700">{generalNote}</p>
                </div>
              )}

              <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 text-sm text-amber-800">
                ⚠️ Stock deducted via <strong>FEFO</strong> (First Expiry First Out) for available items. Cannot be undone.
              </div>
              ...
              <div className="bg-slate-50 border-2 border-slate-300 rounded-2xl p-5 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <p className="font-bold text-slate-800 text-base">Bill Sent to Cashier!</p>
                    <p className="text-xs text-slate-600 mt-0.5">
                      {bill?.billNumber || "Created"} · {rxProp.patientName}
                    </p>
                  </div>
                  {bill?.hasNote && (
                    <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-amber-100 text-amber-700 border border-amber-300 flex-shrink-0">
                      📝 Note flagged
                    </span>
                  )}
                </div>
                      ...
                      {(bill.lines || []).filter(l => l.qtyDispensed > 0).map((l, i) => (
                        <div key={i} className="flex justify-between py-2 text-sm">
                          <span className="text-gray-700">
                            {l.medicationName}
                            <span className="text-xs text-gray-400 ml-1">× {l.qtyDispensed}</span>
                          </span>
                ...
                {/* Out-of-stock items listed on bill */}
                ...
                {/* Note shown on cashier */}
                {bill?.hasNote && (
                  <div className="bg-amber-50 border border-amber-200 rounded-xl px-3 py-2 text-xs text-amber-800">
                    📝 <strong>Cashier note:</strong> {bill.noteContent}
                  </div>
                )}

                {!bill && (
                  <p className="text-sm text-slate-700 text-center">
                    All lines were out of stock — no bill created.
                  </p>
                )}
              </div>
...
export default function PharmacyDashboard() {
  ...
  const [modal,     setModal]     = useState(null); // rx to process
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError("Failed to load dashboard. Check your server connection.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDone = (updatedRx) => {
    setModal(null);
    showToast(`${updatedRx.prescriptionRef} dispensed — bill sent to cashier`);
    load(); // refresh counts + queue
  };
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between fu">
          <div>
            ...
            <h1 className="text-xl font-bold text-gray-800 mt-0.5" style={{ fontFamily: "'Playfair Display', serif" }}>
              {greeting()} 👋
            </h1>
          </div>
          <button onClick={load}
            className="px-3 py-2 bg-white border border-gray-200 rounded-xl shadow-sm hover:shadow-md transition text-sm text-gray-500 flex items-center gap-1.5">
            🔄 Refresh
          </button>
        </div>
        ...
        <div className="rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-4 relative overflow-hidden fu2"
          ...
          <div className="relative">
            ...
            <p className="text-white/60 text-sm mt-1 space-x-2">
              {queue.dispensedToday > 0 && <span className="text-blue-200">{queue.dispensedToday} dispensed today</span>}
              {outOfStock.length    > 0 && <span className="text-red-300">· {outOfStock.length} out of stock</span>}
              {lowStockDrugs.length > 0 && <span className="text-amber-300">· {lowStockDrugs.length} low stock</span>}
            </p>
          </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6 fu4">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  {loading
                    ? "Loading…"
                    : `${pendingQueue.length} prescription${pendingQueue.length !== 1 ? "s" : ""} — click any to review & dispense`}
                </p>
              </div>
                    ...
                    <div key={rx._id}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 truncate">
                          {meds.slice(0, 3).map(l => l.medicationName).join(", ")}
                          {meds.length > 3 && ` +${meds.length - 3} more`}
                        </p>
                        <p className="text-xs text-gray-400 mt-0.5">
                          {time} · {rx.prescriptionRef} · Dr. {rx.doctorName}
                        </p>
                      </div>

                      {/* Action */}
                      <div className="flex flex-col items-end gap-2 flex-shrink-0">
                        <span className={`text-xs font-semibold px-2.5 py-1 rounded-full border flex items-center gap-1 ${s.bg} ${s.text} ${s.border}`}>
                          {s.icon} {s.label}
                        </span>
                        <span className="text-xs font-semibold text-slate-700 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-lg
                          group-hover:bg-slate-700 group-hover:text-white group-hover:border-slate-700 transition">
          ...
          <div className="space-y-5">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <div className="space-y-2">
                {[
                  { label: "Full Queue",     href: "/pharmacy/queue",     icon: "📋", color: "#263238", bg: "#ECEFF1",
                    badge: totalInQueue > 0 ? totalInQueue : null },
                  ...
                  { label: "Stock Alerts",   href: "/pharmacy/inventory", icon: "⚠️", color: "#E65100", bg: "#FFF3E0",
                    badge: (lowStockDrugs.length + outOfStock.length) > 0 ? lowStockDrugs.length + outOfStock.length : null },
                ].map(a => (
                  <a key={a.label} href={a.href}
                    ...
                    {a.badge && (
                      <span className="text-xs font-bold px-2 py-0.5 rounded-full text-white"
                        style={{ background: a.color }}>{a.badge}</span>
                    )}
                        ...
                        <div className="flex items-center justify-between mb-1.5">
                          <div className="flex items-center gap-1.5 min-w-0">
                            ...
                            <span className="text-sm font-medium text-gray-800 truncate">
                              {d.name} <span className="text-gray-400 text-xs">{d.strength}</span>
                            </span>
                          </div>
                          <span className={`text-xs font-bold flex-shrink-0 ml-2 ${isOut ? "text-red-600" : "text-amber-600"}`}>
                            {isOut ? "OUT" : `${total} ${d.unit || "pcs"}`}
                          </span>
                        </div>
                        <div className="w-full bg-gray-100 rounded-full h-1.5">
                          <div className={`h-1.5 rounded-full ${isOut ? "bg-red-400" : "bg-amber-400"}`}
                            style={{ width: `${pct}%` }}/>
                        </div>
                        <p className="text-xs text-gray-400 mt-1">
                          {isOut ? "Reorder immediately" : `Reorder at: ${reorder}`}
                        </p>
                      </div>
                    );
                  })}
            ...
            {expiringSoon.length > 0 && (
              <div className="bg-white rounded-2xl border border-purple-100 shadow-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-purple-100">
                  <h3 className="font-semibold text-gray-800 text-sm">Expiring Within 30 Days</h3>
                </div>
                <div className="divide-y divide-gray-50">
                      ...
                      <div key={entry._id || idx} className="px-5 py-3 flex items-center justify-between">
                        <div className="min-w-0">
                          <p className="text-sm font-medium text-gray-800 truncate">
                            {entry.drug?.name} <span className="text-gray-400 text-xs">{entry.drug?.strength}</span>
                          </p>
                        ...
                        <span className={`text-xs font-bold px-2 py-1 rounded-lg flex-shrink-0 ml-2 ${
                          daysLeft <= 7 ? "bg-red-100 text-red-700" : "bg-purple-100 text-purple-700"
                        }`}>
                          {daysLeft}d left
                        </span>
                      </div>
        ...
        {/* Summary strip */}
        {!loading && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 fu4">
            <h3 className="font-semibold text-gray-800 text-sm mb-4">Prescription Summary</h3>
            <div className="grid grid-cols-5 gap-3">
              {[
                ...
                { label: "Partial Stock", count: queue.partiallyAvailable ?? 0, color: "#fb923c", bg: "#FFF3E0" },
                { label: "Dispensed",     count: queue.dispensedToday     ?? 0, color: "#4ade80", bg: "#ECEFF1" },
                { label: "Cancelled",     count: queue.cancelled          ?? 0, color: "#f87171", bg: "#FFEBEE" },
              ].map(s => (
        ...
        {/* Stock value */}
        {!loading && stockInfo.totalStockValue !== undefined && (
          <div className="rounded-2xl p-5 flex items-center justify-between fu4"
            style={{ background: "linear-gradient(135deg, #ECEFF1, #E8EAF6)" }}>
            <div>
            ...
            <div className="text-right text-sm text-gray-500">
              <p>{stockInfo.totalDrugs ?? 0} active drug SKUs</p>
              {stockInfo.totalExpired > 0 && (
                <p className="text-red-500 font-medium mt-1">{stockInfo.totalExpired} expired entries</p>
              )}
            </div>
          </div>
        )}
