...
const SALES = [
  ...
  },
  {
    ...
    subtotal: 960, discount: 120, total: 840, paid: true, method: "Cash",
  },
  {
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="text-center border-b border-dashed border-gray-200 pb-4">
            ...
            <p className="text-xs text-gray-500 mt-1">Pharmacist: PHC Pharmacy Staff</p>
          </div>
          ...
          <div className="flex justify-between text-sm">
            ...
            <div className="text-right">
              ...
              <span className={`inline-block mt-2 text-xs font-bold px-2 py-0.5 rounded-full ${
                sale.paid ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"
              }`}>
          ...
          <div className="bg-gray-50 rounded-xl p-4 space-y-2">
            <div className="flex justify-between text-sm text-gray-600">
              <span>Subtotal</span><span>LKR {sale.subtotal.toLocaleString()}</span>
            </div>
            {sale.discount > 0 && (
              <div className="flex justify-between text-sm text-green-600">
                <span>Discount (10%)</span><span>- LKR {sale.discount.toLocaleString()}</span>
              </div>
            )}
            <div className="border-t border-gray-200 pt-2 flex justify-between font-bold text-base text-gray-800">
              <span>Total</span>
              <span style={{ color: "#2E7D32" }}>LKR {sale.total.toLocaleString()}</span>
            </div>
            {sale.paid && (
              <div className="text-xs text-gray-400 text-right">Payment: {sale.method}</div>
            )}
          </div>
          ...
          {!sale.paid && (
            <button onClick={onClose}
              ...
              💳 Mark as Paid — LKR {sale.total.toLocaleString()}
            </button>
          )}
...
export default function PharmacyBilling() {
  ...
  const unpaidTotal = SALES.filter(s => !s.paid).reduce((sum, s) => sum + s.total, 0);
  const todaySales = SALES.filter(s => s.date === "15 Feb 2026");
  const todayRevenue = todaySales.filter(s => s.paid).reduce((sum, s) => sum + s.total, 0);
  ...
  return (
    <PharmacyLayout activePage="Sales & Billing">
      ...
      <div className="p-6 space-y-5">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Track pharmacy sales and manage payments</p>
        </div>

        {/* Summary cards */}
        <div className="grid md:grid-cols-4 gap-4">
          {[
            { label: "Today's Revenue", value: `LKR ${todayRevenue.toLocaleString()}`, sub: `${todaySales.filter(s=>s.paid).length} sales`, icon: "💰", color: "#2E7D32", bg: "#E8F5E9" },
            ...
            <div key={s.label} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              ...
              <div className="font-bold text-gray-800" style={{ fontFamily: "'Playfair Display', serif", color: s.color, fontSize: "1.1rem" }}>
                {s.value}
              </div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-amber-700 mt-1">
                {SALES.filter(s=>!s.paid).length} sale(s) pending payment totalling <strong>LKR {unpaidTotal.toLocaleString()}</strong>.
              </p>
            </div>
          ...
          {filtered.map(sale => (
            <div key={sale.id}
              className={`bg-white rounded-2xl border shadow-sm hover:shadow-md transition overflow-hidden ${
                !sale.paid ? "border-amber-100" : "border-gray-100"
              }`}>
              <div className="flex items-center gap-4 px-6 py-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-gray-400 mt-0.5">{sale.date} · {sale.time} · {sale.id} · Rx: {sale.rxId}</div>
                </div>
                ...
                <div className="text-right flex-shrink-0 mr-4">
                  ...
                  {sale.discount > 0 && (
                    <div className="text-xs text-green-600">- LKR {sale.discount.toLocaleString()} disc.</div>
                  )}
                </div>
