...
export default function Register() {
  ...
  // useUsername: false = show email field, true = show username field
  ...
  // When DOB is blank we treat the patient as adult for the UI — the
  // "date of birth required" rule still blocks progress on its own.
  ...
  const handleToggleUsername = () => {
    // Clear both fields when switching so no stale data is sent
  ...
  const validateStep = () => {
    ...
    if (step === 0) {
      ...
      if (!useUsername) {
        ...
        if (!/^[a-zA-Z0-9_]{3,20}$/.test(formData.username.trim()))
          return setError('Username must be 3-20 characters, letters/numbers/underscore only.') || false;
      }
    ...
    if (step === 1) {
      ...
      /* ─── Age-based emergency-contact rule ────────────────────
         Patients under 18 MUST provide both emergency contact
         name and number. Patients 18+ may leave them blank. */
      ...
      // Format check — only validates when a value is present.
      if (formData.emergencyContactNumber.trim() &&
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Send EITHER email OR username — never both
      if (!useUsername) {
        ...
        <div className="w-full max-w-lg">
            ...
            <div className="space-y-4">
              ...
              {!useUsername ? (
                <div>
                  ...
                  />
                  {/* Link to switch to username */}
                  <button
                    type="button"
                    onClick={handleToggleUsername}
                    className="mt-2 text-xs text-blue-600 hover:text-blue-800
                               hover:underline transition">
                    Don't have an email address? Continue with a username →
                  </button>
                </div>
              ) : (
                <div>
                  <InputField
                    ...
                    placeholder="e.g. kamal_99"
                    value={formData.username} onChange={handleChange}
                    hint="3-20 characters, letters/numbers/underscore only. You can add an email later from your profile."
                  />
                  {/* Link to switch back to email */}
                  <button
                    type="button"
                    onClick={handleToggleUsername}
                    className="mt-2 text-xs text-blue-600 hover:text-blue-800
                               hover:underline transition">
                    ← I have an email address, use that instead
                  </button>
                </div>
              )}
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              {/* Info banner shown only when the DOB makes the patient a minor */}
              {isMinor && (
