...
const PatientFeedback = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!hasRating) return setError('Please select a star rating.');
    if (hasDescription && description.trim().length < 10) return setError('Your written review must be at least 10 characters.');
    try {
      setSubmitting(true);
