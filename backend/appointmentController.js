...
export const cancelAppointment = async (req, res) => {
  try {
    ...
    appointment.cancellation = {
      ...
      notifiedAt:  new Date(), // patient already knows — no toast
    };
...
// ══════════════════════════════════════════════════════════
// GET UNSEEN HOLIDAY CANCELLATIONS  (patient)
// Returns every appointment for the logged-in patient that was
// auto-cancelled because the doctor marked a holiday AND that
// the patient has not yet been notified about.
//
// The frontend calls this on every page load of the patient
// portal; each returned record triggers one Apple liquid-glass
// toast. The toast is considered "seen" the moment this
// endpoint responds, so it will not re-appear on refresh.
// (The frontend also persists seen ids in localStorage as a
...
// MARK HOLIDAY (doctor)
//
// When the doctor marks a future date/session as a holiday, every
// patient with a Pending appointment in that blocked window is
// automatically cancelled and tagged with cancellation.source =
// 'holiday'. Those patients see a toast the next time they open
// their portal (see getHolidayCancellations above).
// ══════════════════════════════════════════════════════════
  ...
  try {
    ...
    res.status(201).json({
      ...
      message: cancelledCount
        ? `Unavailability marked. ${cancelledCount} appointment${cancelledCount === 1 ? '' : 's'} were cancelled and patients will be notified.`
