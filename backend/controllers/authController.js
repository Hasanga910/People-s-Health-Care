...
// ── Age helper ─────────────────────────────────────────────
// Returns age in completed years from a date-of-birth value.
...
export const register = async (req, res) => {
  try {
    ...
    // Patients 18 or older may leave those fields blank.
...
export const updateMe = async (req, res) => {
  try {
    ...
    if (patientDetails) {
      ...
      // ── Age-based emergency-contact rule on profile edit ─
      // If the (possibly updated) birthday makes the patient under 18,
      // require emergency contact name + number in the merged result.
      const mergedBirthday = patientDetails.birthday ?? existing.birthday;
      const age = calcAge(mergedBirthday);
      if (age !== null && age < 18) {
        const mergedName =
          patientDetails.emergencyContactName ?? existing.emergencyContactName;
        const mergedNumber =
          patientDetails.emergencyContactNumber ?? existing.emergencyContactNumber;

        if (!mergedName || !String(mergedName).trim())
          return res.status(400).json({
            success: false,
            message: 'Emergency contact name is required for patients under 18 years old',
          });
        if (!mergedNumber || !String(mergedNumber).trim())
          return res.status(400).json({
            success: false,
            message: 'Emergency contact number is required for patients under 18 years old',
          });
