...
export const dispensePrescription = async (req, res) => {
  try {
    ...
    const subtotal = Math.round((drugSubtotal + labSubtotal) * 100) / 100;
    ...
    const bill = await PharmacyBill.create({
      ...
      // Lab test charges
      labCharges,
      labSubtotal: Math.round(labSubtotal * 100) / 100,
      hasLabCharges: labCharges.length > 0,
      labRequestRef: originalRx?.labRequestRef || null,
